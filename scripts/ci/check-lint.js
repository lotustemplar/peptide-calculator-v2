...
function main() {
  ...
  if (!extra.length) {
    console.log("lint: no new/changed non-legacy JS/TS outside scripts/.");
    return;
  }
  ...
  if (extraLint.status !== 0) {
    ...
    fail("SR-CI-001: lint failed on changed/new JS/TS outside the legacy allowlist.");
  }
  console.log("lint: changed/new non-legacy files clean.");
}
