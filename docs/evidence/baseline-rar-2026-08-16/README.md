# Baseline RAR 2026-08-16 — provenance (Stage 0)
...
The uploaded archive was labeled `peptide-calculator-v2.zip` but is a **RAR**. The usable extracted tree from the 2026-09-08/09 planning pass is an **August 2026 rebuild** (`STORAGE_KEY = fitgen-peptide-rebuild-v1`). Core product file mtimes on that tree clustered on **2026-08-16** (UTC).
...
3. **Not deployable as `main`.** Do not overwrite GitHub `main` with the RAR or the extracted tree. Do not revive baseline stub `*-fix.js` files as if they were real patches.
4. **Archive stays outside this repository.** The untouched extract used for planning lived at `/workspace/baseline-compare/baseline/` on the comparison workspace. The full RAR, the complete unvetted tree, and `unzipped/` (partial extract with 0-byte cores) are **not** in git and must not be added.
...
**Default:** no baseline `app.js`, `index.html`, `styles.css`, stub patches, archive temps, binaries, or RAR bytes.
